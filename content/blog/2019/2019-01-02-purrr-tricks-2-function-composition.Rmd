...
This means we can solve the problem in our motivating example by composing a function with pipes:
```{r}
load_files <- . %>% 
  path() %>% 
  dir_ls(regexp = "[.]csv$") %>% 
...
> Composing functions with `magrittr` pipes creates a function in its own right. The composed function is known as a __functional sequence__.

We can inspect the list of functions inside the functional sequence with `magrittr::functions`:
```{r}
